...
function queueAction(group, members, coverageIssues, failureCategory) {
  ...
  if (failureCategory === 'analysis_request_budget_exhausted')
    return 'Rerun the CI workflow to refresh this review after reducing this pull request or raising the bounded Mistral request budget.';
  if (!coverageIssues.length)
    return 'Rerun the CI workflow to refresh this review after correcting the transient analysis failure for this immutable decision unit.';
  if (coverageIssues[0].reason === 'github_rate_limited')
    return 'Rerun the CI workflow to refresh this review after the GitHub API rate limit resets; this immutable decision unit was not fully collected.';
  if (coverageIssues[0].reason === 'github_request_budget_exhausted')
    return 'Rerun the CI workflow to refresh this review after reducing this pull request or raising the bounded GitHub request budget.';
