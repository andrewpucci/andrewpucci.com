...
- Lighthouse runs on every deployment via the `lighthouse` CLI. ADR-0012 defines which absolute thresholds and relative regressions block CI.
...
- **Performance CI is a separate job with a same-runner baseline.** ADR-0012 replaces the original absolute LCP merge gate with a five-run median comparison between the current and base revisions on the same runner. Absolute Lighthouse LCP remains visible and the production p75 objective remains unchanged. Performance score, best practices, SEO, CLS, and TBT continue to use absolute CI thresholds.
