<script lang="ts">
  ...
  function getClientError(): string | undefined {
    if (!inputElement || (!touched && !showValidation) || inputElement.validity.valid) return undefined;

    if (inputElement.validity.valueMissing) {
      return requiredMessage ?? inputElement.validationMessage;
    }
    ...
    if (
      ...
      return invalidMessage ?? inputElement.validationMessage;
    }
  ...
  function getState(): 'default' | 'error' | 'valid' {
    ...
    if ((touched || showValidation) && inputElement?.validity.valid && currentValue.trim()) return 'valid';
    return 'default';
  }
